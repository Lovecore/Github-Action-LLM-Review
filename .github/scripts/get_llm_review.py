...
"""
        ...
        try:
            ...
            # Try Anthropic Claude-like structure
                ...
                'content' in response_data and response_data['content'] and
            ...
            #     review_text = response_data.get("review", "Error: Could not parse 'review' field from LLM response.") # Generic fallback
                ...
                review_text = (
                    "Error: Could not parse review from LLM response using common patterns. Raw response: "
                    f"\n{json.dumps(response_data, indent=2)}"
                )
        except (json.JSONDecodeError, KeyError, IndexError, TypeError) as parse_error:
            review_text = (
                f"Error parsing LLM JSON response: {parse_error}. "
                f"\nRaw response text: \n{response.text}"
            )
